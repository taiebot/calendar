...
class ProposalService
{
    ...
    public function convertProposal(IUser $user, int $proposalId, int $dateId, array $options = []): void
    {
        ...
        // locate selected calendar (or fallback to primary)
        $calendarUri = $options['calendarUri'] ?? null;
        $userCalendar = null;
        ...
        if ($calendarUri !== null) {
            ...
            foreach ($calendars as $calendar) {
                if ($calendar->getUri() === $calendarUri
                    && $calendar instanceof ICreateFromString
     ...
     */
    public function getUserCalendars(IUser $user): array
    {
        $calendars = $this->calendarManager->getCalendarsForPrincipal(
            'principals/users/' . $user->getUID()
        );
    ...
    public function getPrimaryCalendarUri(IUser $user): ?string
    {
        if (method_exists($this->calendarManager, 'getPrimaryCalendar')) {
            $calendar = $this->calendarManager->getPrimaryCalendar($user->getUID());
            if ($calendar !== null) {
                return $calendar->getUri();
            }
        }
        return null;
    }
